...
\begin{algorithm}[tb]
    ...
    \Begin{
        \tcp{Initialise active set of fracture curves.}
        %$ \mathcal{F} := \varnothing $ \\
        $ P_0 := \text{regular $n$-polygon along great circle $C_0$} $ \\
        $ P_0 := P_0, \text{with each vertex moved by $2\pi/n\cdot R\cdot\textit{randSigned}(\,)$ along $C_0$} $ \\
        $ P_0 := P_0, \text{with each vertex moved on unit sphere by $2\pi/n\cdot A\cdot\textit{randSigned}(\,)$ orthogonally to $C_0$} $ \\
        $ Q := \text{random rotation} \in \textit{SO\,$(3)$} $ \\
        $ \mathcal{F} := \{ Q P_0, Q \widebar{P_0} \},\;\text{with $\widebar{P}$ the reversed spherical polygon $P$.}$ \\
