...
\title[Fracturing artefacts into 3D printable puzzles pieces]%
      {\KRedit[Digital workflow for creating 3D puzzles to engage audiences in the interpretation of archaeological artefacts]{Fracturing artefacts into 3D printable puzzles pieces to enhance audience engagement with heritage collections}}
%
...
paleontologists~\cite{AMNH2013}. \KRedit[]{Another application is a megalithic
...
\KRedit{The approach for \MSedit[generating]{producing} solid surfaces for fabrication, which underlies the proposed \MSedit[workshop]{workflow}, deploys algorithms that combine triangular mesh representations with constructive solid geometry (CSG) operations. CSG is a technique commonly used in solid modeling CAD systems. It allows to create a complex surface by using Boolean operators to combine simpler objects. 
...
naturally occuring pottery fragments but represent an interesting
...
\paragraph{Hierarchical frature.}
%
...
%
This isprired our second fracture generation method that
hierarchically subdivides the spherical domain until a target number
of fragments is reached. Fractures occur at random orientations, and
fracture lines are fractal curves to add more realistically looking
raggedness to each edge; see Appendix~\ref{apx:hierarchical-algo} for
more details.
%
Parameters of the algorithm are shown in
...
shows resulting fracture line characteristics for different parameter values.
...
\begin{figure}[h]
  ...
  \subcaptionbox{\label{fig:fracture-type-examples:hierarchical:first}%
    Hierarchical fracture pattern, m: 6, niter: 6, jitter: 0.3, amplitude: 0.01, decay: 0.5, smoothing: 0.0}%
  {\includegraphics[width=0.30\linewidth]{images/fracturelowamplitude}}%
  \hspace{0.2in}%
  \subcaptionbox{Hierarchical fracture pattern, m: 6, niter: 6, jitter: 0.3, amplitude: 0.1, decay: 0.5, smoothing: 1.0}%
  {\includegraphics[width=0.30\linewidth]{images/fracturestandardsmooth}}%
  \hspace{0.2in}\\
 \subcaptionbox{Hierarchical fracture pattern, m: 6, niter: 6, jitter: 0.3, amplitude: 0.2, decay: 0.2, smoothing: 1.0}%
  ...
  \subcaptionbox{\label{fig:fracture-type-examples:hierarchical:last}%
    Hierarchical fracture pattern, m: 6, niter: 6, jitter: 0.1, amplitude: 0.3, decay: 0.5, smoothing: 0.3}%
...
\begin{figure}[h]    
     \centering
      ...
      \subcaptionbox{m: 6, niter: 6, jitter: 0.3, amplitude: 0.2, decay: 0.2, smoothing: 1.0\label{highjitter}}
...
\KRedit{For the Saltdean urn, we generated} \MSedit[14]{16} individual puzzle pieces. \MSedit[Six]{Seven} of these pieces \MSedit[are]{were} retained
...
\TW{This is where we should have, and reference,
  Figures~\ref{fig:pot-with-different-fracture-settings}
  and~\ref{fig:three-shapes-with-different-fractures}.}
...
hierarchical fracture algorithm we use in
Section~\ref{sec:fracture-patterns}.

\TW{TODO: finish this very rough section; stratify terminology
  (fracture polygon; fracture line; fracture; fragment; fractal;
  \dots}
...
\begin{algorithm}[tb]
    ...
    \KwResult{ \\
        $\mathcal{F}$: set of $N$ spherical polygons denoting fracture curves for producing fragments.\\
    }
    \Begin{
        \tcp{Initialise output.}
