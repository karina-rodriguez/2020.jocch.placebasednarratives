...
\KRedit[]{Additionally, there are certain situations when the user will require of various tries before being able to find a suitable puzzle configuration. Figure~\ref{manyexamples} shows various examples generated by for the Saltdean pot archaeological artefact. The puzzles pieces have been generated with 16 random seeds using the same settings of $m$: 6, $n$: 6, $R$: 0.3, $A$: 0.1, $\rho$: 0.5, $\sigma$: 0. As illustrated in the figure, some configurations work better than others in terms of avoiding very small puzzle pieces and cuts which migh be unfeasible for manufacturing given their potentiall to cause the piece to easily break. It is possible to mitigate to a certain extent these effects by rotating the resulting fracture pattern before creating the intersection with the 3D model in order to see if there is a more suitable solution. However, this is mostly a manual process and in most situations it is faster to generate a large number of solutions automatically and then pick up the best. The last row of Figure~\ref{manyexamples} illustrates a good sets of puzzles pieces which will work for manufacturing.
...
\begin{figure}[htb]
  ...
  \includegraphics[width=0.90\linewidth]{images/allselected.jpg}
  \caption{Examples of puzzles produced for the the Saltdean pot scan with a fracture pattern generated with settings $m$: 6, $n$: 6, $R$: 0.3, $A$: 0.1, $\rho$: 0.5, $\sigma$: 0 \label{manyexamples}}
\end{figure}
