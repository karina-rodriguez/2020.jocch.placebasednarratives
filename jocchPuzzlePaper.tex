...
\subcaptionbox{3D scanned urn model\label{step1}}
 ...
 \subcaptionbox{Fabricated puzzle in museum exhibition\label{step3}}
  {\includegraphics[width=.35\linewidth]{images/exhibitmuseum}}\\[-0.5\baselineskip]
  \caption{Workflow for the development of physical puzzles of pottery archaeological artefacts.\vspace*{0.5\baselineskip}}
  \label{fig:teaser}
\end{teaserfigure}
%
...
objects in a museum gallery. The reason for this is that a puzzle
...
complex nature of the process. The contribution of this paper is a novel 
digital worfklow for the design and fabrication of 3D puzzles which overcomes
these limitations. The input
...
deployed it to create a puzzle activity of the Saltdean urn, which is
...
demonstrate its applicability to other shapes. The significance of this 
research is that it eases the task
...
% We have added in section 5  a more detailed evaluation of the exhibit.
...
% We have added a paragraph in section 4.3 to describe how a user might need to try alternative versions of the puzzle in order to find the most suitable to their needs. This will address the limitations suggested by the reviewers that a given results might not produced the desired results. We also added table 2 with processing timings for a sample of 3D meshes to give an idea how long it takes to compute a puzzle piece. Furthermore, we added an additional figure (Figure~\ref{manyexamples}) which illustrates different version of running the algorithm and how failure cases might look like (these are mostly very small pieces). We added a related discussion on this topic and describe some possible ways to mitigate these failure cases. The faster solution is to generate many alternative versions with a low-poly 3D mesh and choose the best one visually which can later be applied to a high-poly 3D model.
...
% Our response: we previously mentioned that these references were not relevant to our work which is why we did not include them in the revision (either of the conference or of the journal paper). Given the reviewer's comments we had another look at the references and still believe our initial assessment is correct. The suggested references deal with the recording of cultural heritage assets over time and multi-source data integration. In particular they deal with recording CH at an architectural scale (reference 1 and 3), including rural landscape and urban landscapes as well as the use of Building Information Modeling (BIM) for historic environments and structures (reference 2). In our case, we deal with one single CH object and we are not particularly focusing on its documentation, conservation nor preservation. Neither are we interested to record the object through time. We instead aim to create a physical puzzle for an exhibit. If the intended link to these references is the use of 3D technologies, we believe that this will mean introducing a new section that reviews the use of 3D technologies for CH. However, this will require an in-depth review of the area and will possibly include hundreds of references to research in the last few years, which makes this suggestion impractical. We would like to clarify our paper is not intended as a state of the art paper, but rather we are making a contribution to a specific problem in this larger domain. 
...
educational
activities~\cite{Neely2013,Scopigno2014,Neumuller2014,doi:10.1111/cgf.12781}.
...
technology~\cite{Neely2015,Miles2015}.
...
\KRedit[]{As shown in
  ...
  possible to create alternative versions of the puzzle from the same
  geometry. This ensures the user has some control over the best
  version of the puzzle for a given 3D mesh. To achieve this, the
  best approach is to test the patterns with a simplified version of
  the 3D mesh. Once the user has selected the best pattern, this can
  ...
  model. Table~\ref{table-procesingtimes} presents the computation
  times for various 3D models with different number of faces in a
  laptop with Intel Core i9 processor and 32 GB DDR4.}

\KRedit[]{Users might require various attempts before finding a
  ...
  generated for the Saltdean pot archaeological artefact. The
  puzzle pieces have been generated with 16 random seeds using the
  ...
  bottom row of Figure~\ref{manyexamples} illustrates a good set of
  puzzle pieces that have a suitable size and can be manufactured.
...
kind of tray or wooden box in order to prevent pieces from
falling down and possibly breaking. Lastly, it has been noticed that
...
artefact. Finally, we thank the 3D printing technicians Trevor Taylor, 
