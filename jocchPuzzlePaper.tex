...
\KRedit[]{Users might require of various tries before finding a suitable puzzle configuration. Although fragment shapes vary across the different runs, they tend to be of consistent characteristics. Figure~\ref{manyexamples} shows various examples generated by for the Saltdean pot archaeological artefac. The puzzles pieces have been generated with 16 random seeds using the same settings of $m$: 6, $n$: 6, $R$: 0.3, $A$: 0.1, $\rho$: 0.5, $\sigma$: 0. As illustrated in the figure, some configurations work better than others to produce feasible puzzles which are manufacturable and that work well in the physical domain. This includes avoiding very small puzzle pieces and cuts which might be unfeasible for manufacturing or cuts that can cause the physical piece to easily break. It is possible to mitigate these effects to a certain extent by rotating the resulting fracture pattern before creating the intersection with the 3D model in order to see if there is a more suitable solution. However, this is mostly a manual process and in most situations it is faster to generate a large number of solutions automatically and then pick up the best. The bottom row of Figure~\ref{manyexamples} illustrates a good sets of puzzles pieces that have a suitable size and can be manufactured.
...
\begin{figure}[htb]
  ...
  \includegraphics[width=0.88\linewidth]{images/allselected.jpg}
  \caption{\label{manyexamples}%
    ...
    $R$: 0.3, $A$: 0.1, $\rho$: 0.5, $\sigma$: 0. The
      ...
      for twelve randomly chosen random seeds. The bottom row
      shows the pieces generated with the patterns
      displayed in the top row, third
      column. The puzzle pieces were generated with a 3D model
      with a higher resolution (30,000 faces).}}
\end{figure}
