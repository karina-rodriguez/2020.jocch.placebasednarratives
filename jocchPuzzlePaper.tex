...
\title[Digital workflow for creating 3D puzzles]%
      ...
      {\MS{Could it be something like: Workflow and fabrication of 3D puzzles to support interpretation of archeaological artefacts}
%
...
fragments \MSedit[]{of broken pottery}. The contribution of this paper is a novel \KRedit[digital]{} worfklow
...
demonstrate the workflow, we \MSedit[deploy]{deployed} the proposed method to create a 3D
puzzle of \MSedit[an artefact,]{} the Saltdean urn, \MSedit[for]{which is exhibited at} the Archaeology
Gallery of the Brighton Museum and Art Gallery. \MS{Karina I added the following to do it more generic as well} \MSedit[]{The workflow is also used with further artefacts in order to demonstrate its applicability to other shapes and types of artefacts.}The significance of
this research is that it eases the task of creating puzzle-like
activities and maintaining them \MSedit[]{in the long term} within a busy museum gallery.
\end{abstract}
...
generating the puzzle. \MSedit[]{Section~\ref{res} presents further examples where the algorithms have been applied in order to showcase the applicability of the workflow to other shapes and types of artefacts.} Section~\ref{eva} discusses the evaluation of
...
computer games, virtual reality and the film industry. In this area, predefined fracture patterns are often computed, while more novel solutions focus on creating fast approximations which are computational efficient and offer flexible control of fragment generation \cite{Schvartzman:2014:FAB:2556700.2556713,Hahn:2016:FAB:2897824.2925902,Zhu:2015:SRB:2809654.2766942,Muller:2013:RTD:2461912.2461934}. Of relevance to the heritage field is the geometric analysis of a dataset of fracture patterns observed in wall paintings excavated at Akrotiri, Greece \cite{Shin:2012:ASF:2362402.2362404}. This analysis suggests pottery fragments in a hierarchical fracture pattern, where fragments break into two pieces recursively along cracks nearly orthogonal to previous ones.}


Most of the proposed solutions \KRedit{in literature} aim to create puzzles consisting only
of the required individual pieces. \MS{As this really isn't an opposition to the previous sentence, I think that we should erase this "however"} \MSedit[However,]{In our case,} we aim to
create a permanent \MSedit[]{hands-on} exhibit \KRedit[of a pottery vase]{} for a busy museum
...
the use of magnets \KRedit{or other attachment mechanisms which are placed both} on \KRedit[its]{the core's} surface and on each puzzle piece.
...
\KRedit{The approach for \MSedit[generating]{producing} solid surfaces for fabrication, which underlies the proposed \MSedit[workshop]{workflow}, combines algorithms generating fracture patterns with constructive solid geometry (CSG) operations. CSG is a technique commonly used in solid modeling CAD systems. It allows to create a complex surface by using Boolean operators to combine simpler objects. 
...
steps. The generator then produces:]{Once the fracture pattern has been generated, \MSedit[this]{it} is used to \MSedit[generate]{produce} the puzzle pieces.}
...
\begin{figure}[h]
  ...
  {\includegraphics[width=0.30\linewidth]{images/puzzlepiece}}
    ...
    \caption{\KR{Need a better caption}\MS{Could it be: Puzzle fragment generation through fracture pattern?}Process of generating each puzzle piece using fracture pattern}
\end{figure}
...
Figure~\ref{fig:sphereb}, each \KR{do I use the word "section" before, not sure this is correct}\MS{Karina the word section hasn't been used, the word fracture is used above} \MSedit[section of the fractured sphere]{fracture of the sphere}
...
\KRedit{For the Saltdean urn, we generated} \MSedit[14]{16} individual puzzle pieces.\MS{In figure 11a we show 16 pieces, so I changed it to reflect that. The number of pieces for the base is actually 7, so I changed that as well} \MSedit[Six]{Seven} of these pieces \MSedit[are]{were} retained
...
required blind holes (see Figure~\ref{fig:teaser}-c). \MS{This figure is not relevant. Should we refer to 11a instead which shows all printed pieces?}
%
\begin{figure}[h]
  \centering
...
once, it was split into \MSedit[eight]{four} \MS{Karina it was 4 pieces not 8} sections to achieve better printing
...
texture. Figure~\ref{fig:assembled} \KRedit[demonstrates the samples that were 3D printed using PLA filaments in different colours. The sample on the top right of the image has been chosen for the puzzle pot. The sample has been coated with the described mixture and painted using acrylic colours. The pieces are currently being post-processed to achieve the same visual quality as the samples.]{demonstrates the 3D puzzle assembled \MSedit[once all the pieces and the core have been post-processed]{before the final post-processing steps}. \MSedit[Afterwards]{Finally}, an artist painted the pieces \MSedit[using the colours tested]{} to resemble the original pot and the core (see Figure~\ref{fig:painti}). \MSedit[]{Lines to provide clues about the shape of the puzzle pieces and facilitate assembly were also painted on the core.} The final interactive exhibit \MSedit[is]{was} then placed at the museum gallery (see Figure~\ref{fig:allprintedassembled} ).}
...
the puzzle pieces to ``sit on''. \KRedit{We \MSedit[then]{also} explore alternative fracture patterns in order to generate different types of puzzle pieces}. An algorithm is then proposed to
generate the puzzle fragments or shards and the \KRedit{attachments, such as the} blind-holes for the
...
\KR{we need to rephrase this, as I don't think these really are the greatest challenges.... the challenge really is the fabrication.}
\MS{Karina I have added the following, please rephrase it as you wish}
...
printing technology.]{Some of the challenges that we experienced and could be faced when creating similar puzzle activities are related to  the creation of a watertight mesh out of a partially scanned pottery artefact. Another challenge is the selection of the right fragmentation pattern in order to comply with the design requirements. However, the biggest challenge is related to the fabrication of the puzzle pieces and core as tolerances due to the layer size of the 3D printing technology as well as post-processing steps (e.g. pasting pieces together, using certain materials) might greatly affect the final appearance and functionality of the exhibit. That is why prototyping and iterative testing is crucial, in order to understand how measurements, settings and processes might need to change in order to successfully produce a satisfying result.}
