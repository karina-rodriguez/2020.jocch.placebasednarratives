...
settings. In particular, \sout{it} the paper expands the theoretical context in which the research is conceptualised, as well as presents a digitisation pipeline \sout{which allow} allowing to capture a wide variety of creative narrative forms based on photogrammetry. 
...
\subsection{Digitisation of 3D scene for AR} \sout{There were} Several considerations
were taken into account \sout{made} in order to digitise the physical artwork using 3D technologies. \sout{These}
Decisions were mostly driven by the fact that \sout{the content} the crafted material was going to be rendered as
...
for their inclusion in the AR experience during \sout{this stage} the first iterations of the project.

Hence, the first task was to produce the 3D models of the \sout{boxes} box-houses as well as to
digitise all content inside the boxes. A mixed approach was used to create the
3D models of the houses. We trialed \sout{to} two methods which can potentially produce suitable results for generating this type of experiences. These include 3D modelling and photogrammetry for large-scale digitisation. Both of these will be explained in the following subsections.

\subsection{3D modelling}
Given the fact that all houses are based on a physical box
...
In Blender \sout{the Blender program}, simple primitives, such as cubes and planes, were used
to create the shape of each house. The geometry of each house began \sout{geometries of the houses each began} as
