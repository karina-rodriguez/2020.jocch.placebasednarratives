...
a vivid, present depiction of heritage that can be deployed by future
generations as well \MS{I have added this about future generations}.
Moreover, the printed element of the map can also serve as a physical
...
interpretative narratives \MS{I would add as it is very relevant to
current situation " while representing different voices and embracing
...
Hove (UK) and then expanded to five more schools in the area. \MS{I
added the iteration} The children firstly engaged through \sout{an
artistic} a creative process to generate place-based narratives of their
...
experience and share. \MS{added this about sharing experience}
...
craft created during the narrative process. Thereafter,
...
the fact that the process must be scalable to deal with large amount of
crafted elements, and that the crafted material was going to be rendered
as augmented content on the map. Hence, important requirements were to
design workflows that provided content from every child, produced
high-quality content and that enable to produce good quality
representations which enabled to keep high framerates. This meant that
the box-houses and their content were prioritised for digitisation
amongs other crafted elements. Other crafted objects,  such as those
...
experimented with a semi-automated image-based workflow. The main
motivation for this is that it will allow us to process a wider variety
of shapes without having to deal with modelling every customisation done
to the crafted items. The main challenges which needed to be addressed
was to ensure the visual quality of the items by capturing both the
shape and the colour accurately. 


All box-houses were created from a similar cardboard box as a starting
point, which were customised using paper of various colours and crayons.
Although areas coloured using crayons displayed some shine when catching
beams of light, most material was fairly non-reflective. We followed a
similar process than \cite{Gaiani_2017} to ensure colour fidelity of the
artefacts. This ensured that the 3D models which resulted from the
process had a good and uniform quality in particular for the cardboard
textures.


We made use of the X-Rite ColorChecker for the colour characterization
process in the lighting environment in which we performed the
digitisation process. The X-Rite ColorChecker automatically detects the
...
rotated $90\degree$, generating in total between 54 - 60 images. We also
made use of a measuring bar which uses circular cross-type targets. This
allows to introduce scale to the 3D model when this is later processed.
...
exporting them as a JPEG to be used during the image-based 3D modelling
process. The sets of images are then automatically processed by a Python
script which uses the Agisoft Metashape API  \cite{Metashapesite}. This
script reads the images from a folder, processes a 3D model, detect the
targets captured in the image, scales and centres the model as shown in
Figure \ref{fig:metashape}. All of this process is fully automatic.


% \begin{figure}[ht] \centering
% \includegraphics[width=1\linewidth]{images/metashape.png} 
% \caption{3D model of the box-house generated through a photogrammetry automated
% process} \label{fig:metashape}  
% \end{figure}


Thereafter, a manual process is required to remove the excess geometry,
...
the production of 3D meshes to be of suitable size/quality for
distributing them over the web.  % Moreover, the textures had to be
optimised for web delivery as the initial images were % produced with
very high resolution and file size. This was done by running % them
through the imageoptim web service \cite{ImageOptim}.   %Furthermore,
all images had to be resized to dimensions which were powers of two
(e.g. % 2048x2048, 512x1024) to facilitate loading.  The images produced
used a final % resolution of 96dpi. For this, the glTF file format was
used for encoding the 3D models and their animation for the AR
experience. glTF (GL Transmission Format) is a common specification for
the efficient transmission and loading of 3D scenes and models,
especially by web applications \cite{khronos}. It reduces both the size
...
resulting colour fidelity of the 3D models being produced by the
digitisation process. For this, we made the assumption that all areas of
the 3D model which display a cardboard texture will have very similar
colours.

Our method uses small patches (80 by 80 pixels) which are extracted from
textures of two different 3D models, as shown in Figure
\ref{fig:texturepatches}. We select areas that correspond to the
non-decorated areas of the cardboard boxes as we assume these two areas
must be similar in colour. 


We make use of the $\Delta$E (Delta E) measurement to understand the
differences perceived between colours from the two different textures.
$\Delta$EE is a metric for understanding how the human eye perceives
...
means that colours are exact opposite. We hypothesised that getting
values between 1-10 meant the changes in texture colour will be
perceptible only through close observation or at a glance in the worst
case scenario.

In order to calculate this metric, we firstly have to select the best
colours in the texture patch to compare. For this, we look for the
dominant colours on a given patch using K-means cluster analysis. By
using this method, we produce a distribution of the most 5 dominant
colours as illustrated in Figure \ref{fig:distribution}. We then test
...
To calculate $\Delta$E for the colour in the texture patches, we firstly take 
the top 5 most dominant colours of a texture
patch and convert them to the CIELAB colour space. CIELAB colour space,
...
Table presents a comparison across the five most dominant colours for the texture $ A $ and $ B $. Beside all colours present it a similarity of under 3.5, meaning there is only a perceptible difference amongst the textures when very closely inspected. It should also be noted that when comparing a colour to the colour with a similar percentile, this are very similar in their $\Delta$E value. 
...
\bottomrule
\end{tabular}
\caption{$\Delta$E value comparison for the top 5 colour  values for each texture patch}
\end{table}

The evaluation results demonstrate that our process for generating 
low resolution textures for the 3D model produce good and uniform quality 
in terms of colour. This allows us to
produce a standardised process which can be deployed for the large-scale
